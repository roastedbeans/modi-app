...
import util
import qualcomm.diagcmd as diagcmd
    ...
    def parse_lte_pdcp_ul_cip(self, pkt_header, pkt_body, args):
        ...
        if pkt_version == 1:
            ...
            for x in range(n_subpackets):
                ...
                if subpkt_id == 0xC3:
                    if subpkt_version == 0x1A or subpkt_version == 0x28:
                        ...
                        if self.parent:
                            self.parent.logger.log(logging.WARNING, 'Unexpected PDCP UL Cipher Data Subpacket version %s' % subpkt_version)
                        pos += subpkt_size
                        continue
            ...
            if self.parent:
                self.parent.logger.log(logging.WARNING, 'Unknown PDCP UL Cipher Data packet version 0x{:02x}'.format(pkt_version))
